...
const int out_min_deg = -90;
const int out_max_deg = 90;
//const int out_min_deg = -90;
//const int out_max_deg = 90;
...
public:
	...
	void setDirTE() {
		...
		//Dir = "C:\\Users\MasayaDohino\Desktop\cavelab\simulationHairModel\incidenceLayer_withSig\(210nm, 152cell)\TM\Ns\NTFF";
	...
	void setDirTM() {
		...
		//ÉVÉ~ÉÖÉåÅ[ÉVÉáÉìÇ≈ìæÇΩÉfÅ[É^ÉtÉ@ÉCÉãåQÇ÷ÇÃÉpÉX

		Dir = "C:/Users/MasayaDohino/dataset1/";
	...
	void getPlot(int lam, int in_deg) {
		...
		/*outïî
		 out_degÇÕéUóêîgÇÃäpìx
		 out_min ~ out_max ÇÕ 0~180?
		out_d_deg = 1 : äpìxä‘äu
		 out_num = (out_max_deg - out_min_deg) / out_d_deg;
		*/


		/*inïî
		const int in_min_deg = ;
		const int in_max_deg  =;
		const int in_d_deg = 5;
		const int in_num = (in_max_deg - in_min_deg) / in_d_deg;
		const int cellNum = (in_num + 1) * (out_num + 1);
		const double offset = 0.2
	...
	void draw_axis() {
		...
		*/
		...
		for(int i=out_min_deg; i<=out_max_deg; i+= 30){
			...
			drawBitmapString(GLUT_BITMAP_HELVETICA_12, to_s(i), -1.0+0.5*offset, y);
