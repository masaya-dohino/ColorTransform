		...
		for (int i = 0; i < in_num; i++) {
			for (double k = 0.0; k < in_d_deg; k += 1) {
				for (int j = 0; j < out_num; j++) {
					...
					double d = r * (k + 0.5);
					MyColor c1 = (1.0 - d) * Color(i, j);
					MyColor tmp1 = d * Color(i + 1, j);
					c1 = c1 + tmp1;

					MyColor c2 = (1.0 - d) * Color(i, j + 1);
					MyColor tmp2 = d * Color(i + 1, j + 1);
					c2 = c2 + tmp2;
	...
	void getPlot(int lam, int in_deg) {
		...
		while (fp) {
			...
			if ((out_deg < out_max_deg) && (out_deg >= out_min_deg)) {
				...
				MyColor tmp = rgb->CIE_RGB(rgb->getXYZ(lam, 3 * ref));
				Color(in, out) = Color(in, out) + tmp;
			}
			out_deg++;
		}
	}
...
//		drawBitmapString(GLUT_BITMAP_HELVETICA_12, "theta(deg)", -0.1, -dh -0.1);
	...
	void drawCell(int i, int j) {
		...
		for (double k = 0.0; k < in_d_deg; k += 1) {
			...
			double d = r * (k + 0.5);
			MyColor c1 = (1.0 - d) * Color(i, j);
			MyColor c11 = d * Color(i + 1, j);
			c1 = c1 + c11;
			MyColor c2 = (1.0 - d) * Color(i, j + 1);
			MyColor c22 = d * Color(i + 1, j + 1);
			c2 = c2 + c22;
			MyColor c = 0.5*(c1 + c2);
